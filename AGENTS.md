...
tests/wifi-loc-selftest.sh                自测脚本（四份配方，75 条断言，全部用桩）
...
5. **两个方向都自动化，但只有「离开」会回落到默认位置。** 进入已知位置是自动的；特征设备未确认时**立即**回落到默认位置（默认位置就是 DHCP，在哪儿都能用），并在**同一轮内**等 15 秒复探一次——命中就静默切回，仍不命中就静默留下；而**在某个已知位置内发现目标设备不符时，只通知、不回落**——那说明这张网本身变了，擅自动作会把静态设置悄悄换成 DHCP。两者靠**特征设备**（我在哪张网上）与**目标设备**（这张网还是我设置里期待的那张吗，默认取特征设备）区分，见 `docs/2026-09-16-decision-model-design.md`。
...
```sh
bash tests/wifi-loc-selftest.sh              # 四份全跑，实测 196 秒
...
2. **判定层** —— 38 条断言，约 2 分钟：设计文档 N1–N8、N10–N12（**N9 不在这里**，见第 4 条）：`ok` / `broken` 去重 / 先切换后核对目标 / **立即回落 + 轮内复探** / 复探命中**静默切回** / 身份不符才通知 / 旧格式状态文件。两条技巧：**必须像 agent 一样带 `--apply --notify`**（否则通知与状态写入根本不会走到——第一次写这份配方时正是漏了这两个参数，被 14 条失败打回来）；**「复探命中」那条路径**要用桩 `arp` 让特征地址前 8 次查询报无条目（正好是一轮失败探测的成本：4 次尝试 × 2 次查询），才能确定性地复现它。用 `WLC_CONFIRM_DELAY=1` 把 15 秒等待压到 1 秒。
...
- 判定逻辑（脚本层）：进入方向真实切换；特征设备未确认时**立即回落到默认位置**，轮内复探命中就**静默切回**、仍不命中就静默留下；目标设备不符时**只通知、不回落**；通知文案不含地址与 MAC。
- 四份配方全通过（10 + 38 + 14 + 13 = 75 条断言，0 失败），已提交为 `tests/wifi-loc-selftest.sh`。
...
- **新流程（立即回落 + 轮内复探）**：由 `decision` 的 38 条与 `reconfirm` 的 13 条断言覆盖，**真实环境尚未复测**——需要再切一次热点才能确认。
...
连续两轮只出现过一次，且那次是真离开——加上「自己切换会清空邻居表」实测**不成立**（切换后那一轮 1–3 秒就命中），这两条一起成了删掉跨轮安全阀的依据（见设计文档 §8）。另外两处未知：目标设备**真的**下线（R3S 掉电）未实测，只用未应答地址模拟过；新流程尚未在真实环境复测。

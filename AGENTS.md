...
tests/wifi-loc-selftest.sh                自测脚本（五份配方，105 条断言，全部用桩）
...
12. **通知去重靠状态文件，其含义是「用户当前是否已被告知一个异常状态」。** `ok` / `default` 一律重置；只有**真的送达**了才写 `broken`。这样手动跑（不带 `--notify`）不会污染状态，投递失败也不会被记成已送达，两者都不会让 agent 漏掉本该发的通知。**回落不再产生通知**，所以它只写 `default`（位置真的切了就如实记，不能因为通知失败而写成「还在别处」）。旧格式（裸值、或带 `miss=` 行）都按「没收到过通知」处理，**无需迁移**。**另有一行 `pending=feature-mismatch`**，只在「特征地址上换了设备」那条通知投递失败时写：它表示「欠着一条通知」，下一次能观察到同一异常（当前在默认位置、该地址上仍是别的设备）的轮次重试，送达后清空。`state` 与 `pending` 各记各的，位置该怎么记就怎么记，不因为投递失败而谎报。改动这段逻辑前先读 `docs/2026-09-16-decision-model-design.md` 第 3 节的状态转移表。
...
```sh
bash tests/wifi-loc-selftest.sh              # 五份全跑，实测 207 秒
...
4. **通知状态机** —— 20 条断言：目标在场不通知、`broken` 只通知一次、手动跑（不带 `--notify`）不写状态也不吞掉以后的通知、投递失败不记账且下一轮重试、**特征未确认时静默回落**，以及 **「换了设备」那条通知投递失败 → 写 `pending=` 记欠账 → 可观察时重试并清空**（N17）。
...
- 五份配方全通过（17 + 6 + 20 + 49 + 13 = **105 条断言，0 失败**，实测 207 秒），已提交为 `tests/wifi-loc-selftest.sh`。

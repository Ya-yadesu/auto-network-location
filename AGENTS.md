...
- **新流程（第一遍扫描即回落 + 轮内复探）**：
  - **「短暂缺席 + 复探切回」已有真实首例**（2026-09-16 15:51:48–15:52:21，无人干预）：`15:51:48` 读到 `Home` → 探测 8 秒无应答 → `15:51:56` 回落到 `Automatic` → 使用者随即连回家里的 Wi-Fi → `15:52:11` 复探、`15:52:19` 命中 → **静默切回 `Home`** → `15:52:21` 目标在场、`state=ok`，**全程 0 条通知**。注意这一轮跑的是**改动之前**的版本（回落仍需走满 4 次尝试），而且它验证的是「短暂缺席」而不是「真离开」。
  - **回落决策提前**（设计文档 §8 晚间改定）：由 N13/N14 覆盖，**真实环境尚未复测**。
...
连续两轮只出现过一次，且那次是真离开——加上「自己切换会清空邻居表」实测**不成立**（切换后那一轮 1–3 秒就命中），这两条一起成了删掉跨轮安全阀的依据（见设计文档 §8）。另外三处未知：目标设备**真的**下线（R3S 掉电）未实测，只用未应答地址模拟过；**真离开**（设备持续缺席到 `we have left`）在真实环境里一次都没出现过——`15:51:48` 那一轮以「复探命中、切回 `Home`」收尾；**提前回落之后的真实一轮**也还没跑过。

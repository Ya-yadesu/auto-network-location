...
**先不做安装层。** 安装继续由 `README.md` 的步骤承担，本轮只补文档。理由：

1. **收益与频率不匹配**：这套东西一年迁移不了几次，而且只服务一个人。安装脚本的价值来自
   「迁移频率 × 做错的代价 × 是否给别人用」——前两项都很低。
2. **`WatchPaths` 基本判了 C 的死刑**：见 §4.C，`brew services` 表达不了本项目唯一的触发机制。
3. **7 个卡点里有 6 个是自证的**：跑一次干跑就知道位置读没读对、特征设备命中没有（§3.2）。
   安装脚本主要能省的是打字，不是判断。

**本轮实际做的**（全部是文档）：`README.md` 与 `README.en.md` 各补三块——权限与重启的事实、
建位置前的备份与回滚、迁移照做清单（每步带验收命令）。代码一行没动。

**重开的条件**（任一条出现，就回到 §4 重新评估）：

- 要在**第二台**机器上再装一次（频率变了）；
- README 的步骤被误操作过一次（代价真的发生了，尤其是 §3.2 的卡点 1）；
- 要给别人用（受众变了，此时 §4.C 的退化形态也要一起重估）；
- 安装步骤在未来某次 macOS 更新后失效（例如 `networksetup` 的行为改变）。

...
三份都用**桩**：`osascript` 让通知只记账不弹出，`scselect` 让切换只记账、**不改动本机位置**，两者放进临时 `PATH`；判定层还额外用一个**桩 `arp`**（见第 2 条）。它们写在 `/tmp` 下、**没有提交进仓库**，会话结束不留存，需要时按下面的清单重建（**不要**照抄 `docs/2026-09-16-decision-model-plan.md` 的 Task 2 Step 1——那是设计变更前的版本）。

1. **配置层** —— 10 条断言：目标字段的默认与校验，含「名字含空格时不算重名」与「越界 IP 被跳过」两条回归。
2. **判定层** —— 38 条断言，约 2 分钟：设计文档 N1–N12 全覆盖（`ok` / `broken` 去重 / 先切换后核对目标 / **立即回落 + 轮内复探** / 复探命中**静默切回** / 身份不符才通知 / 旧格式状态文件）。两条技巧：**必须像 agent 一样带 `--apply --notify`**（否则通知与状态写入根本不会走到——第一次写这份配方时正是漏了这两个参数，被 14 条失败打回来）；**「复探命中」那条路径**要用桩 `arp` 让特征地址前 8 次查询报无条目（正好是一轮失败探测的成本：4 次尝试 × 2 次查询），才能确定性地复现它。用 `WLC_CONFIRM_DELAY=1` 把 15 秒等待压到 1 秒。
...
- **新流程（立即回落 + 轮内复探）**：由 38 条断言覆盖，**真实环境尚未复测**——需要再切一次热点才能确认。

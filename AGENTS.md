...
- **真实端到端（2026-09-16 14:00–14:02，使用者手动切网、agent 全程无人干预）**：
  - 切到手机热点后：`14:00:11` 特征设备缺席 → `miss 1 of 2`（安全阀挡住，什么都不做）；`14:01:18` 第二轮仍缺席 → **回落到 `Automatic` + 一条通知**，`state` 变 `default`、`miss` 归零。**从第一次未确认到回落 74 秒**。
  - 连回家里的网络后：`14:02:11` 读到 `Automatic` + 特征设备在场 → `14:02:13` **自动切回 `Home`** → `14:02:15` 目标设备核对通过、`state` 回 `ok`，整个过程**没有再发通知**。
  - **切换本身会瞬断网络**：`scselect` 重配接口，使用者实测每一次切换都伴随一次短暂中断（回落、切回各一次）。这是设计内的代价，不是故障。

...
**已验证**：`--apply` 的进入方向（2026-09-16 实测）——先 `scselect Automatic`（DHCP），脚本探到特征设备、识别为 `Home`，`--apply` 真实调用 `scselect` 切到 `Home`，位置回到该有的静态 IPv4 + 指定网关/DNS、IPv6 Off，退出码 0。

**尚未验证**：
- 「离家后特征设备从邻居表消失」这一半。必须用真实移动或连到另一个网络验证，不能靠推理；要留意邻居表可能残留旧条目，导致把「已经不在家」误判成「还在家」。
